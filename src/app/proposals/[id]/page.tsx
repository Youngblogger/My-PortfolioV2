    ...
    return (
      <div className="min-h-screen py-20 px-6 flex items-center justify-center">
        <div className="glass rounded-2xl p-12 text-center max-w-md">
          ...
          <h3 className="text-lg font-bold text-white mb-2">Failed to Load Proposal</h3>
    ...
    return (
      <div className="min-h-screen py-20 px-6 flex items-center justify-center">
        <div className="glass rounded-2xl p-12 text-center max-w-md">
          ...
          <p className="text-muted text-sm mb-6">This proposal does not exist or has been removed.</p>
          <Link href="/proposals">
    ...
    <div className="min-h-screen py-20 px-6">
      <div className="max-w-4xl mx-auto">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="text-xs text-muted border border-white/10 rounded-md px-2 py-0.5">
                  v{proposal.version}
                </span>
              </div>
        ...
        <div className="space-y-6">
          ...
          >
            <h2 className="text-sm font-semibold text-gold mb-3 tracking-widest uppercase">Scope of Work</h2>
            <p className="text-white text-sm leading-relaxed whitespace-pre-wrap">{proposal.scope_of_work}</p>
          </motion.div>
          ...
          {proposal.deliverables.length > 0 && (
            <motion.div
              ...
              className="glass rounded-2xl p-6"
            >
              <h2 className="text-sm font-semibold text-gold mb-3 tracking-widest uppercase">Deliverables</h2>
          ...
          <motion.div
            ...
            className="glass rounded-2xl p-6"
          >
            <h2 className="text-sm font-semibold text-gold mb-3 tracking-widest uppercase">Timeline</h2>
            <p className="text-white text-sm leading-relaxed">{proposal.timeline_description}</p>
          </motion.div>
          ...
          {proposal.milestones && proposal.milestones.length > 0 && (
            <motion.div
              ...
              className="glass rounded-2xl p-6"
            >
              <h2 className="text-sm font-semibold text-gold mb-3 tracking-widest uppercase">Milestones</h2>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10 text-left text-muted text-xs uppercase tracking-wider">
                  ...
                  <tbody>
                    {proposal.milestones.map((ms, i) => (
                      <tr key={i} className="border-b border-white/5 last:border-0">
                        <td className="py-3 pr-4 text-white font-medium">{ms.name}</td>
                        <td className="py-3 pr-4 text-muted">{ms.description}</td>
                        <td className="py-3 pr-4 text-muted">{ms.due_date ? formatDate(ms.due_date) : "—"}</td>
                        <td className="py-3 text-gold font-semibold text-right">{formatCurrency(ms.amount_ngn)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          {proposal.payment_schedule && proposal.payment_schedule.length > 0 && (
            <motion.div
              ...
              className="glass rounded-2xl p-6"
            >
              <h2 className="text-sm font-semibold text-gold mb-3 tracking-widest uppercase">Payment Schedule</h2>
              <div className="space-y-3">
                {proposal.payment_schedule.map((ps, i) => (
                  <div key={i} className="flex items-center justify-between py-2 border-b border-white/5 last:border-0">
                    <div className="flex items-center gap-4">
                      <span className="text-sm text-white">{ps.event}</span>
                      <span className="text-xs text-muted">{ps.percentage}%</span>
                    </div>
                    <span className="text-sm text-gold font-semibold">{formatCurrency(ps.amount_ngn)}</span>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <motion.div
            ...
            className="glass rounded-2xl p-8 text-center gold-border"
          >
          ...
          {proposal.terms_and_conditions && proposal.terms_and_conditions.length > 0 && (
            <motion.div
              ...
              className="glass rounded-2xl p-6"
            >
              <h2 className="text-sm font-semibold text-gold mb-3 tracking-widest uppercase">Terms &amp; Conditions</h2>
              <ul className="space-y-2">
          ...
          {proposal.valid_until && (
            <div className="text-center text-sm text-muted">
              Proposal valid until <span className="text-white font-medium">{formatDate(proposal.valid_until)}</span>
            </div>
          )}
          ...
          {proposal.versions && proposal.versions.length > 1 && (
            <motion.div
              ...
              className="glass rounded-2xl p-6"
            >
              <h2 className="text-sm font-semibold text-gold mb-4 tracking-widest uppercase">Version History</h2>
              <div className="space-y-3">
                  ...
                  >
                    <div>
                      ...
                      {v.changes_description && (
                        <p className="text-xs text-muted mt-0.5">{v.changes_description}</p>
                      )}
                    </div>
                    <span className="text-xs text-muted shrink-0 ml-4">{formatDate(v.created_at)}</span>
                  </div>
                ))}
              </div>
            </motion.div>
