          ...
          <div className="grid lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              >
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                    ...
                    <span className="text-white font-medium">{state.service.title}</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                    <span className="text-muted">Package</span>
                    <span className="text-white font-medium">{state.package.name}</span>
                  </div>
                  <div className="flex items-center justify-between py-2 border-b border-white/5">
                    <span className="text-muted">Delivery</span>
                    <span className="text-white font-medium">To be agreed</span>
                  </div>
                  {state.addOns.length > 0 && (
                    <div className="py-2 border-b border-white/5">
                      <div className="flex items-start justify-between">
                        <span className="text-muted shrink-0 mt-1">Add-ons</span>
                        <div className="flex flex-wrap gap-1.5 justify-end">
                          {(showAllAddOns ? state.addOns : state.addOns.slice(0, 3)).map((a) => (
                            <span key={a.id} className="text-white font-medium text-sm border border-white/10 rounded-md px-2 py-0.5">
                              {a.name}
                            </span>
                          ))}
                          {!showAllAddOns && state.addOns.length > 3 && (
                            <span className="text-muted text-sm border border-white/10 rounded-md px-2 py-0.5">
                  ...
                  <div className="flex items-center justify-between py-2">
                    ...
                    <span className="text-base font-bold text-white">₦{grandTotalNgn.toLocaleString()}</span>
                  </div>
                </div>
              </motion.div>
              ...
              >
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-gold/50 focus:ring-1 focus:ring-gold/20 transition-all duration-300"
                />
              ...
              >
                ...
                <div className="grid md:grid-cols-2 gap-3 text-sm">
                  <div>
                    <span className="text-[10px] text-muted uppercase tracking-wider">Full Name *</span>
                    <input type="text" name="full_name" value={form.full_name} onChange={handleChange}
                      placeholder="Your full name"
                      className="w-full mt-0.5 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-gold/50 focus:ring-1 focus:ring-gold/20 transition-all duration-300" />
                ...
                {showClientDetails && (
                  <div className="mt-3 pt-3 border-t border-white/5 grid md:grid-cols-2 gap-3 text-sm">
                    <div>
                      <span className="text-[10px] text-muted uppercase tracking-wider">Country</span>
                      <input type="text" name="country" value={form.country} onChange={handleChange}
                        placeholder="Your country"
                        className="w-full mt-0.5 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-gold/50 focus:ring-1 focus:ring-gold/20 transition-all duration-300" />
                    </div>
                    <div>
                      <span className="text-[10px] text-muted uppercase tracking-wider">City</span>
                      <input type="text" name="city" value={form.city} onChange={handleChange}
                        placeholder="Your city"
                        className="w-full mt-0.5 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm placeholder:text-white/20 focus:outline-none focus:border-gold/50 focus:ring-1 focus:ring-gold/20 transition-all duration-300" />
                    </div>
                    <div className="md:col-span-2">
                      <span className="text-[10px] text-muted uppercase tracking-wider">Address</span>
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              {!form.preferred_start_date && (
                ...
                >
                  <label className="block text-xs text-muted mb-2">Preferred Start Date</label>
              ...
              >
                    ...
                    {[
                      ...
                      <label key={option.value}
                        className={`flex items-start gap-3 rounded-xl p-3.5 cursor-pointer transition-all duration-200 ${
                          form.payment_type === option.value ? "bg-gold/10 ring-1 ring-gold/30" : "bg-white/5 hover:bg-white/10"
                        }`}
                      >
                    ...
                    {[
                      { value: "paystack", label: "Paystack", badge: "NGN", icon: (
                        <svg className="w-7 h-7" viewBox="0 0 32 22" fill="none">
                          <rect x="0.5" y="0.5" width="31" height="21" rx="3" className="fill-white/10 stroke-white/20" />
                          <rect x="3" y="4" width="12" height="2" rx="1" className="fill-white/30" />
                          <rect x="3" y="8" width="8" height="2" rx="1" className="fill-white/20" />
                          <rect x="3" y="12" width="10" height="2" rx="1" className="fill-white/20" />
                          <circle cx="22" cy="13" r="4" className="fill-gold/30" />
                          <circle cx="25" cy="13" r="4" className="fill-gold/20" />
                        </svg>
                      )},
                      { value: "flutterwave", label: "Flutterwave", badge: "NGN/USD", icon: (
                        <svg className="w-7 h-7" viewBox="0 0 32 22" fill="none">
                          <rect x="0.5" y="0.5" width="31" height="21" rx="3" className="fill-white/10 stroke-white/20" />
                          <circle cx="10" cy="11" r="5" className="fill-gold/20" />
                          <path d="M17 7l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-white/40" />
                          <path d="M23 7l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-white/40" />
                        </svg>
                      )}
                    ].map((gateway) => (
                      <label key={gateway.value}
                        className={`flex items-center gap-3 rounded-xl p-3 cursor-pointer transition-all duration-200 ${
                          form.payment_gateway === gateway.value ? "bg-gold/10 ring-1 ring-gold/30" : "bg-white/5 hover:bg-white/10"
                        }`}
                      >
                        ...
                        <div className="flex-1">
                          <span className="text-sm text-white font-medium">{gateway.label}</span>
                        </div>
                        <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${
                          form.payment_gateway === gateway.value ? "border-gold" : "border-white/20"
                        }`}>
                          {form.payment_gateway === gateway.value && <div className="w-2 h-2 rounded-full bg-gold" />}
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className="space-y-2 text-sm mb-4">
                  ...
                  <div className="flex justify-between pt-2 border-t border-white/5">
                    ...
                    <span className="text-gold text-lg font-bold">₦{displayTotal.toLocaleString()}</span>
                  </div>
                ...
                {/* Terms */}
                <p className="mt-4 pt-4 border-t border-white/5 text-[10px] text-muted leading-relaxed text-center">
                  By clicking <span className="text-white">Complete Secure Payment</span>, you agree to our{" "}
