  ...
  const columns: Column<PortfolioItem>[] = [
    {
      ...
      header: "Title",
      render: (item) => (
        <Link href={`/admin/portfolio/${item.id}`} className="text-sm font-medium text-white hover:text-gold transition-colors">
          {item.title}
        </Link>
      ),
    },
    ...
    },
    {
      key: "client_name",
      header: "Client",
      render: (item) => <span className="text-sm text-muted">{item.client_name || "—"}</span>,
      hideOnMobile: true,
    },
    {
      key: "status",
      ...
      render: (item) => (
        ...
        >
          {item.is_featured ? (
            ...
            <svg className="w-5 h-5 text-muted hover:text-gold/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
    ...
    },
    {
      key: "sort_order",
      header: "Sort Order",
      render: (item) => <span className="text-sm text-muted">{item.sort_order}</span>,
      hideOnMobile: true,
    },
    {
      key: "actions",
