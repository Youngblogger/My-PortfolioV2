...
const statusConfig: Record<string, { label: string; color: string }> = {
  ...
  accepted: { label: "Accepted", color: "bg-green-500/10 text-green-400" },
  rejected: { label: "Rejected", color: "bg-red-500/10 text-red-400" },
  cancelled: { label: "Cancelled", color: "bg-orange-500/10 text-orange-400" },
...
function statusBadge(status: string) {
  const cfg = statusConfig[status] || { label: status.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()), color: "bg-white/5 text-white" };
  return (
    ...
    return (
      ...
      >
        <div className="space-y-1">
          ...
          <div className="animate-pulse bg-white/5 rounded-lg h-5 w-96" />
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="glass rounded-2xl p-6 space-y-4">
              <div className="animate-pulse bg-white/5 rounded-lg h-6 w-48" />
              <div className="space-y-3">
                {Array.from({ length: 5 }).map((_, j) => (
                  <div key={j} className="animate-pulse bg-white/5 rounded-lg h-8 w-full" />
                ))}
              </div>
            </div>
          ))}
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="glass rounded-2xl p-6 space-y-4">
              ...
              <div className="space-y-3">
                {Array.from({ length: 5 }).map((_, j) => (
                  <div key={j} className="animate-pulse bg-white/5 rounded-lg h-10 w-full" />
                ))}
              </div>
            </div>
          ))}
        </div>
        <div className="animate-pulse bg-white/5 rounded-2xl h-20 w-full" />
      </motion.div>
    );
  }
      ...
      <div className="grid lg:grid-cols-3 gap-6">
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                    ...
                    <motion.div
                      ...
                      className="w-full max-w-[40px] bg-gold-gradient rounded-t-md"
        ...
        <motion.div variants={itemVariants} className="glass rounded-2xl p-6">
          ...
          {Object.keys(ordersByStatus).length === 0 ? (
            <p className="text-sm text-muted">No order data available.</p>
          ) : (
            <div className="space-y-3">
          ...
          {enrollmentTrends.length === 0 ? (
            ...
            <div className="flex items-end justify-between gap-2 h-48">
              {enrollmentTrends.map((item, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-2 h-full justify-end">
                    <span className="text-[10px] text-gold font-medium">{item.count || 0}</span>
                    <motion.div
                      initial={{ height: 0 }}
                      animate={{ height: `${height}%` }}
                      transition={{ duration: 0.8, delay: i * 0.1, ease: "easeOut" }}
                      className="w-full max-w-[40px] bg-gradient-to-t from-purple-500/60 to-purple-400/30 rounded-t-md"
                      style={{ minHeight: height > 0 ? undefined : 0 }}
                    />
                    <span className="text-[10px] text-muted truncate w-full text-center">
                      {item.month}
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <motion.div variants={itemVariants} className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Latest Proposals</h2>
            <Link href="/admin/proposals" className="text-xs text-gold hover:text-gold/80 transition-colors">
              View all
            </Link>
          </div>
              ...
              {recentProposals.slice(0, 5).map((proposal, i) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-white font-medium truncate">{proposal.client}</p>
                    <p className="text-xs text-muted truncate">{proposal.service}</p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0 ml-3">
                    {statusBadge(proposal.status)}
                    <span className="text-xs text-muted">{formatDate(proposal.created_at)}</span>
                  </div>
                </motion.div>
              ))}
            </div>
        ...
        <motion.div variants={itemVariants} className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Latest Payments</h2>
            <Link href="/admin/payments" className="text-xs text-gold hover:text-gold/80 transition-colors">
              View all
            </Link>
          </div>
          {recentPayments.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentPayments.slice(0, 5).map((payment, i) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-white font-medium truncate">{payment.reference}</p>
                    <p className="text-xs text-muted truncate">{payment.client}</p>
                  </div>
                  <div className="flex items-center gap-3 shrink-0 ml-3">
                    <span className="text-sm font-semibold text-white">{formatNgn(payment.amount_ngn)}</span>
        ...
        <motion.div variants={itemVariants} className="glass rounded-2xl p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/admin/clients" className="text-xs text-gold hover:text-gold/80 transition-colors">
              View all
            </Link>
          </div>
          {recentRegistrations.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentRegistrations.slice(0, 5).map((user, i) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-white font-medium truncate">{user.full_name || "Unknown"}</p>
                    <p className="text-xs text-muted truncate">{user.email}</p>
                  </div>
                  <span className="text-xs text-muted shrink-0 ml-3">{formatDate(user.created_at)}</span>
                </motion.div>
              ))}
            </div>
          )}
      ...
      <motion.div variants={itemVariants} className="glass rounded-2xl p-6">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gold/10 text-gold border border-gold/20 hover:bg-gold/20 transition-all duration-300 text-sm font-medium"
          >
          ...
          <Link
            href="/admin/projects"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 transition-all duration-300 text-sm font-medium"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-purple-500/10 text-purple-400 border border-purple-500/20 hover:bg-purple-500/20 transition-all duration-300 text-sm font-medium"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <Link
            href="/admin/clients"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-green-500/10 text-green-400 border border-green-500/20 hover:bg-green-500/20 transition-all duration-300 text-sm font-medium"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
            </svg>
          ...
          <Link
            href="/admin/settings"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-white/5 text-muted border border-white/10 hover:bg-white/10 hover:text-white transition-all duration-300 text-sm font-medium"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
