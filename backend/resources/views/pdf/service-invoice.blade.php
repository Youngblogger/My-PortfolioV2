  ...
  <div class="invoice-wrapper">
    ...
    <div class="header">
      ...
      <div class="title-section">
        ...
        <div class="invoice-status">{!! \App\Helpers\PdfHelper::statusBadge($invoice->status) !!}</div>
      </div>
    </div>
    ...
    <div class="info-row">
      <div class="info-block">
        <h3>Bill To</h3>
        <div class="name">{{ $billing['full_name'] ?? $profile?->full_name ?? $user?->email ?? 'N/A' }}</div>
        <div class="detail">{{ $billing['email'] ?? $user?->email ?? '' }}</div>
        @if(!empty($billing['phone']) || !empty($profile?->phone))
          <div class="detail">{{ $billing['phone'] ?? $profile?->phone }}</div>
        @endif
        @if(!empty($billing['company']))
          <div class="detail">{{ $billing['company'] }}</div>
        @endif
        @if(!empty($billing['address']) || !empty($billing['city']) || !empty($billing['state']) || !empty($billing['country']))
          <div class="detail">
            @php $addr = []; @endphp
            @if(!empty($billing['address'])) @php $addr[] = $billing['address']; @endphp @endif
            @if(!empty($billing['city'])) @php $addr[] = $billing['city']; @endphp @endif
            @if(!empty($billing['state'])) @php $addr[] = $billing['state']; @endphp @endif
            @if(!empty($billing['country'])) @php $addr[] = $billing['country']; @endphp @endif
            {{ implode(', ', $addr) }}
          </div>
        @endif
      </div>
      <div class="info-block">
        <div class="info-grid">
          ...
          <div class="item">
            ...
            <div class="value">{{ ucfirst($invoice->payment_type ?? 'Full') }}</div>
          </div>
          <div class="item">
    ...
    <table class="items">
      ...
      <tbody>
        ...
        <tr>
          <td>
            <strong>{{ $order->package?->name ?? 'Service Package' }}</strong>
            @if($order->service || $order->projectType)
              <br><span style="font-size:8px;color:#a1a1aa;">
                {{ $order->service?->title ?? '' }}{{ $order->service && $order->projectType ? ' — ' : '' }}{{ $order->projectType?->title ?? '' }}
              </span>
            @endif
          </td>
          <td>1</td>
          <td class="amt">{{ \App\Helpers\PdfHelper::formatCurrency($currency === 'NGN' ? $order->package_price_ngn : $order->package_price_usd, $currency) }}</td>
          <td class="amt">{{ \App\Helpers\PdfHelper::formatCurrency($currency === 'NGN' ? $order->package_price_ngn : $order->package_price_usd, $currency) }}</td>
        </tr>
        ...
        @if($order->addOns && $order->addOns->count() > 0)
          @foreach($order->addOns as $addOn)
            ...
            @endphp
            <tr>
              <td>{{ $addOn->name ?? $addOn->addOn?->name ?? 'Add-on' }}</td>
              <td>1</td>
              <td class="amt">{{ \App\Helpers\PdfHelper::formatCurrency($addOnPrice, $currency) }}</td>
              <td class="amt">{{ \App\Helpers\PdfHelper::formatCurrency($addOnPrice, $currency) }}</td>
            </tr>
          @endforeach
        @endif
    ...
    <div class="totals">
      <table>
        <tr class="subtotal">
          ...
          <td class="amount">{{ \App\Helpers\PdfHelper::formatCurrency($subtotal, $currency) }}</td>
        </tr>
        @if($discount > 0)
        <tr class="discount">
          <td class="label">Discount{{ $invoice->discount_code ? ' (' . $invoice->discount_code . ')' : '' }}</td>
          <td class="amount">-{{ \App\Helpers\PdfHelper::formatCurrency($discount, $currency) }}</td>
        </tr>
        @endif
        @if($tax > 0)
        <tr>
          <td class="label">Tax</td>
          <td class="amount">{{ \App\Helpers\PdfHelper::formatCurrency($tax, $currency) }}</td>
        </tr>
        @endif
        <tr class="grand-total">
          <td class="label">Grand Total</td>
          <td class="amount">{{ \App\Helpers\PdfHelper::formatCurrency($total, $currency) }}</td>
        </tr>
        @if($amountPaid > 0)
        <tr>
          <td class="label">Amount Paid</td>
          <td class="amount" style="color:#059669;">{{ \App\Helpers\PdfHelper::formatCurrency($amountPaid, $currency) }}</td>
        </tr>
        @endif
        @if($balance > 0)
        <tr class="balance">
          <td class="label">Remaining Balance</td>
          <td class="amount">{{ \App\Helpers\PdfHelper::formatCurrency($balance, $currency) }}</td>
        </tr>
        @elseif($amountPaid > 0)
        <tr class="balance positive">
          <td class="label">Balance</td>
          <td class="amount">{{ \App\Helpers\PdfHelper::formatCurrency(0, $currency) }}</td>
        </tr>
        @endif
      </table>
    </div>
        ...
        <tbody>
          @foreach($payments as $payment)
            <tr>
              ...
              <td>{{ ucfirst($payment->gateway ?? 'N/A') }}</td>
              <td class="amt">{{ \App\Helpers\PdfHelper::formatCurrency($currency === 'NGN' ? ($payment->amount_ngn ?? 0) : ($payment->amount_usd ?? 0), $currency) }}</td>
    ...
    {{-- Terms & Conditions --}}
    @if(!empty($order->proposals) && $order->proposals->isNotEmpty() && $order->proposals->first()->terms_and_conditions)
      <div class="notes-section" style="border-left-color:#a1a1aa;">
        <h3>Terms &amp; Conditions</h3>
        @foreach($order->proposals->first()->terms_and_conditions as $term)
          <p>• {{ $term }}</p>
        @endforeach
      </div>
    @else
      <div class="notes-section" style="border-left-color:#a1a1aa;">
        ...
        <p>All prices are in {{ $currency === 'NGN' ? 'Nigerian Naira (₦)' : 'US Dollars ($)' }}.</p>
